...
rm -rf $path/to_move
mkdir $path/to_move
cp $path/{Contact.cpp,Contact.hpp} $path/to_move
upper=$(echo "$1" | tr '[:lower:]' '[:upper:]')
echo "$path/to_move/Contact.cpp $path/to_move/Contact.hpp" | xargs sed -i "" "s/Contact/$1/g"
echo "$path/to_move/Contact.cpp $path/to_move/Contact.hpp" | xargs sed -i "" "s/CONTACT/$upper/g"
