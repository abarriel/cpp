...
bool FragTrap::rangedAttack(std::string const & target) {
   ...
   std::cout << this->type << " " << this->name << " attacks " << target
    << "with a rangedAttack(fragTrap) attack causing " << this->rangedAttackDamage <<  " points of damage ! " << std::endl;
    return true;
}
...
void FragTrap::extendedEnergy(int energy) {
    this->maxEnergyPoints += energy; 
}
...
FragTrap::~FragTrap(void) {
    std::cout << this->name << " I'M DEAD I'M DEAD OHMYGOD I'M DEAD! (FragTrap dead)" << std::endl;
}
